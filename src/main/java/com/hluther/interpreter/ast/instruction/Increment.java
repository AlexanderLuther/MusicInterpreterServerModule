 ...
 * @author helmuth
 */
public class Increment extends Node implements Instruction{
    
    private Instruction id;

    public Increment(Instruction id, int row, int column) {
        super(row, column);
        this.id = id;
    }
    ...
    public Object analyze(TypeTable typeTable, SymbolTable symbolTable, Stack<String> scope, AnalysisError errors){
        ...
        if(tempType != SymbolType.NOT_FOUND && (tempType == SymbolType.STRING || tempType == SymbolType.BOOLEAN)){
            errors.addSemanticError(new MError("Incremento sobre una variable de tipo "+typeTable.get(tempType).getName(), "Incremento", super.getRow(), super.getColumn()));
        }
        return null;
    }
